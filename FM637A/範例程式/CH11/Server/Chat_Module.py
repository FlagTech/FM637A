...
def youtube_to_wav(url):
    ...
    print(f'{url} {video_title} {minutes}分{seconds}秒')
    if minutes > 50:
        return video_title,'time length error'
    print('download---',end='')
    error_info = ''
    ...
    delete_file(file_name) # 刪除 mp4 檔
    print('ok')
    return video_title,error_info,file_name_wav

# ==============音樂撥放器===============
def player(args):
    """
    Args:
        **kwargs: user_text:string
    Returns:reply_text, cmd, error
    """
    music_name =  args.get("music_name", None)
    error = ''
    cmd = {'cmd_name': '', 'cmd_args': {}}
    # try:
    music_url = get_music_url(music_name)
    try:
        music_name,error,file_name_wav = youtube_to_wav(music_url)
        if error == 'time length error':
            error += '\ntime length error == player_machine() '
            reply_text = '播放失敗'
        else:
            reply_text = f'正在為您播放{music_name}'
            cmd = {'cmd_name': 'music', 'cmd_args': {}}
    except Exception as e:
        error += f'\n{e} == player_machine() 播放失敗'
        reply_text = '播放失敗'
    # except Exception as e:
    #     error += f'\n{e} == player_machine() 無法提取歌名 '
    return reply_text, cmd, error

def led_function(args):
    error = ''
    color_name = args.get("color_name", None)
    RED = args.get("RED", None)
    GREEN = args.get("GREEN", None)
    BLUE = args.get("BLUE", None)
    RED, GREEN, BLUE = RED.replace(' ', ''), GREEN.replace(' ', ''), BLUE.replace(' ', '')
    if color_name == '關燈':
        reply_text = '已關燈'
    else:
        reply_text =  f'已調整燈光為{color_name}'
    cmd = {'cmd_name': 'RGB', 'cmd_args': {'red': RED, 'green': GREEN, 'blue': BLUE}}
    return reply_text, cmd, error

# ==============音量控制===============
def control_volume(args):
    with open("db.txt", "r") as db_file:
        db = int(db_file.read())
        if db == '': db = 0
    print(f'原始音量：{db}')
    volume = args.get("volume", None)
    control_db = abs(int(args.get("db", 1)))
    if volume == '小聲':
        db += control_db * (-5)
    elif volume == '大聲':
        db += control_db * (5)
